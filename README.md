...
- **Factual consistency (Aug 4 2026):** evaluated via an adapted GPT-4o-mini methodology, not the official PlainQAFact metric (see Decision 13 for why). Result: ~0.96 overall consistency, with elaboration claims (0.856-0.862) notably less consistent than simplification claims (0.972-0.980) -- matching PlainQAFact's own documented finding that elaborative explanations are more hallucination-prone. Official PlainQAFact metric: environment fully built and verified on a real cloud GPU (RunPod, RTX PRO 6000), pipeline proven end-to-end on a 2-record test (internal_mean 0.9025), full 20-record run pending completion in a stable session before Sep 4 -- see Decision 13 for the explicit caveat that the 2-record result must not be treated as a completed validation. 1 figure.
