...
Every major retrieval-augmented generation system retrieves on semantic similarity alone. Lewis et al. (2020) established the foundational RAG architecture; Karpukhin et al. (2020) formalized dense passage retrieval as its backbone; Izacard and Grave (2021) extended this with multi-passage fusion. None of these systems carry any notion of whether the content they retrieve is appropriate for a given user's literacy level, because none were designed to. Treating simplification as a post-hoc step applied after generation does not resolve this gap; You and Guo (2026) documented that this approach introduces factual errors, since simplification performed on already-generated content lacks access to the source material's full context. The correct point to intervene is upstream, at retrieval, not downstream, after generation has already committed to a specific framing of the content.
...
The remainder of this paper proceeds as follows. Section 2 reviews related work in RAG foundations, biomedical question-answering benchmarks, plain-language and health-literacy research, and health-literacy measurement frameworks. Section 3 describes ORACLE's methodology: the four-stage pipeline, the retrieval corpus, literacy-band classification, the retrieval and generation mechanism, and the evaluation metrics. Section 4 details experimental setup and reproducibility. Section 5 presents results. Section 6 discusses these findings and their implications, and Section 7 concludes.

## 2. Related Work

### 2.1 RAG Foundations

Lewis et al. (2020) introduced retrieval-augmented generation, combining a parametric generation model with a non-parametric retrieval step to ground generation in retrieved evidence rather than relying solely on a model's internal parameters. The architecture treats retrieval as literacy-agnostic by design: a single retrieval pass, conditioned only on the query's semantic content, serves every user identically, because nothing in the query encoding represents who is asking or what they can parse. Karpukhin et al. (2020) formalized dense passage retrieval as the encoder architecture underlying this approach, mapping queries and documents into a shared vector space and ranking candidates by similarity within that space. ORACLE builds directly on this backbone but intervenes at two specific points Karpukhin et al.'s original formulation does not address: the query encoding step, where a literacy band is estimated before retrieval runs, and the candidate-pool selection step, where the search space itself is restricted to that band rather than searched in full and filtered afterward. Izacard and Grave (2021) introduced Fusion-in-Decoder, improving how a generation model synthesizes evidence across multiple retrieved passages rather than conditioning on a single best match. This is a genuine advance for generation quality, but it inherits the same literacy-agnostic retrieval stage as Lewis et al.'s original formulation: better fusion of retrieved passages does not address which passages were retrievable in the first place, which is precisely the stage ORACLE modifies.
