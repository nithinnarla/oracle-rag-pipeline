...
**Decision:** Evaluate Stage 4 factual consistency using an adapted GPT-4o-mini-based methodology (src/factual_consistency_eval.py), not the official PlainQAFact metric (You & Guo, 2026, arXiv 2503.08890) directly.
...
**Planned validation:** The official PlainQAFact metric will be run separately in an isolated cloud GPU environment (RunPod/Lambda Labs, ~$5-20, no risk to this project's local environment) on the same source/summary pairs, once a dedicated session is available before the Sep 4 JBI deadline. The two scores will be compared as an internal validation check -- if they correlate well, this adapted script's results gain credibility as a fast proxy; if they diverge substantially, only the official metric's results will be reported in the paper.
