...
**Validation status (Aug 6 2026, partial): environment built and verified working; full 20-record run incomplete.**
...
**What remains incomplete:** transferring the full 20-record dataset to the cloud instance and running it. Every attempt failed due to persistent session/connection instability during transfer (not a dependency, configuration, or code problem -- the identical pipeline that succeeded on 2 records would very likely succeed on 20 given a stable connection). This is a mechanical blocker, not a research or engineering gap.

**Explicit methodological note -- do not shortcut this:** the 2-record result must NOT be reported as a completed comparison against C's 20-record adapted result. A 2-record sample is not statistically meaningful and drawing any conclusion from its proximity to C's ~0.96 aggregate (e.g. "B validates C") would be a false-precision claim a reviewer should and would flag. The full 20-record official run must be completed before any B-vs-C comparison is stated in the paper. Until then, this decision documents a verified-working-but-incomplete validation, and the paper's evaluation section should cite only C's adapted result, explicitly labeled as adapted (not official), with this incomplete validation noted as ongoing work rather than as supporting evidence.
